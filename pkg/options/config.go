...
import (
	...
	"github.com/HappyLadySauce/Eino-Agent-CLI/pkg/utils/homedir"
)
...
// userConfigDir returns ~/basename, e.g. ~/eino on Unix.
// userConfigDir 返回 ~/basename，例如在 Unix 上为 ~/eino。
...
func findDefaultConfigFile(basename string) (string, bool, error) {
	cwd, err := os.Getwd()
	if err != nil {
		return "", false, fmt.Errorf("cannot get working directory: %w", err)
	}
