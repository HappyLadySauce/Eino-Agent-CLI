...
func TestLoadViperConfigExplicitFile(t *testing.T) {
	...
	if err := loadViperConfig("eino", cfgPath); err != nil {
		t.Fatalf("loadViperConfig() error = %v", err)
	}
...
func TestLoadViperConfigRelativePathResolvesFromCWD(t *testing.T) {
	...
	if err := loadViperConfig("eino", cfgName); err != nil {
		t.Fatalf("loadViperConfig() error = %v", err)
	}
...
func TestLoadViperConfigHomeDirFallback(t *testing.T) {
	...
	homeConfigDir := filepath.Join(homeDir, "eino")
	if err := os.MkdirAll(homeConfigDir, 0o700); err != nil {
		t.Fatalf("os.MkdirAll() error = %v", err)
	}
	settingsPath := filepath.Join(homeConfigDir, "settings.json")
	content := `{"model": {"EINO_BASE_URL": "https://home.example.com", "EINO_MODEL": "home-model"}}`
	if err := os.WriteFile(settingsPath, []byte(content), 0o600); err != nil {
		t.Fatalf("os.WriteFile() error = %v", err)
	}
	...
	if got, want := viper.GetString("model.EINO_BASE_URL"), "https://home.example.com"; got != want {
...
func TestLoadViperConfigCwdTakesPriorityOverHomeDir(t *testing.T) {
	...
	if err := os.WriteFile(cwdSettings, []byte(`{"model": {"EINO_MODEL": "from-cwd"}}`), 0o600); err != nil {
		t.Fatalf("os.WriteFile() error = %v", err)
	}
	...
	if err := os.WriteFile(homeSettings, []byte(`{"model": {"EINO_MODEL": "from-home"}}`), 0o600); err != nil {
		t.Fatalf("os.WriteFile() error = %v", err)
	}
	...
	if got, want := viper.GetString("model.EINO_MODEL"), "from-cwd"; got != want {
...
func TestLoadViperConfigExplicitFileFailsWithoutFallback(t *testing.T) {
	...
	err = loadViperConfig("eino", "missing.json")
	if err == nil {
		t.Fatal("loadViperConfig() error = nil, want non-nil")
	}
	if got := viper.GetString("model.EINO_MODEL"); got != "" {
		t.Errorf("viper.GetString(model.EINO_MODEL) = %q, want empty (no fallback load)", got)
	}
}
...
func TestAddConfigFlagAddsFlagToFlagSet(t *testing.T) {
	...
	AddConfigFlag(fs, "Eino-Agent-CLI")
...
func TestAddConfigFlagBindsEnvWithBasenamePrefix(t *testing.T) {
	...
	fs := pflag.NewFlagSet("t", pflag.ContinueOnError)
	AddConfigFlag(fs, "Eino-Agent-CLI")
	t.Setenv("EINO_AGENT_CLI_FOO_BAR", "from-env")
